...
>
> **Word choice, on purpose:** this project does not use the word "schema." A tag file is not a schema in the database/JSON-Schema sense — there are no types, no constraints, no validation rules beyond name-matching. It is just a plain-English list of tag names, matched by name against CSV column headers. Call it a **tag file**, and call what's inside it a **description** (as in, "the tag file's description of the entity"). If you catch the word "schema" anywhere in this repo going forward, that's stale wording to fix, not a synonym to preserve.

---

## 0. One-paragraph purpose

SecretLead is a **standalone, offline, tag-file-driven CRUD and CSV-import tool for Windows.** The owner writes a tag file describing any kind of record — not just contractors — and SecretLead either imports a matching CSV in bulk or lets records be keyed in by hand, into a store that lives entirely in the app's local AppData and never touches a network. The pitch is not "an offline CSV importer for one dataset." It's: **import your data once, and from that point on the app architecturally cannot make a network call, ever, for anything.** That claim — not any single bundled dataset — is the actual product.
